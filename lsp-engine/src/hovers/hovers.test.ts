...
test('Liquid hover documentation for schema variables', () => new Promise<void>((resolve) => {
  const child = startLspServer();
  let step = 0;
  ...
  new LSPMessageReader(child.stdout!, (res) => {
    ...
    if (step === 0 && res.id === 1) {
      ...
      expect(res.result).toBeDefined();
      expect(res.result.contents).toBeDefined();
      expect(res.result.contents.value).toContain('**Variable:** `user.first_name`');
      ...
      expect(res.result.contents.value).toContain('**Type:** `dropdown` (Options: "Active", "Inactive")');
      ...
      expect(res.result).toBeDefined();
      expect(res.result.contents).toBeDefined();
      expect(res.result.contents.value).toContain('**Variable:** `user.items[0].title`');
      expect(res.result.contents.value).toContain('**Type:** `string`');
